...
@testset "normalize_string (Unicode normalization etc.)" begin
...
@testset "Unicode normalization forms (Unicode SA#15)" begin
    @testset "1. Canonical equivalence" begin
    ...
    @testset "2. Compatibility Equivalence" begin
    ...
    @testset "4. Canonical Composites" begin
    ...
    @testset "5. Multiple Combining Marks" begin
    ...
    @testset "6. Compatibility Composites" begin
...
@testset "issue #5939  uft8proc character predicates" begin
    let
        alower=['a', 'd', 'j', 'y', 'z']
        ulower=['α', 'β', 'γ', 'δ', 'ф', 'я']
        @testset "lowercase letters" for c in vcat(alower,ulower)
            @test islower(c) == true
        ...
        @testset "uppercase letters" for c in vcat(aupper,uupper)
        ...
        @testset "lowercase, uppercase, nocase letters" for c in alphas
             @test isalpha(c) == true
             @test isnumber(c) == false
        end
        ...
        @testset for c in anumber
             @test isdigit(c) == true
             @test isnumber(c) == true
        end
        @testset for c in unumber
             @test isdigit(c) == false
             @test isnumber(c) == true
        end

        alnums=vcat(alphas,anumber,unumber)
        @testset for c in alnums
             @test isalnum(c) == true
        ...
        @testset for c in vcat(apunct,upunct)
             @test ispunct(c) == true
             @test isalnum(c) == false
        end

        @testset for c in vcat(alnums,asymbol,usymbol,apunct,upunct)
            @test isprint(c) == true
            @test isgraph(c) == true
        ...
        aspace = [' ']
        acntrl_space = ['\t', '\n', '\v', '\f', '\r']
        @testset for c in vcat(aspace,uspace)
            @test isspace(c) == true
            @test isprint(c) == true
            @test isgraph(c) == false
        end

        @testset for c in vcat(acntrl_space)
            @test isspace(c) == true
        ...
        @testset for c in vcat(acontrol, acntrl_space, latincontrol)
            @test iscntrl(c) == true
            @test isalnum(c) == false
            @test isprint(c) == false
            @test isgraph(c) == false
        ...
        @testset "non-latin1 controls" for c in ucontrol
            if c!=Char(0x0085)
                @test iscntrl(c) == false
                @test isspace(c) == false
                @test isalnum(c) == false
...
@testset "up-to-date character widths (#3721, #6939)" begin
...
@testset "handling of embedded NUL chars (#10958)" begin
...
@testset "Get full coverage of isspace function" begin
    @test isspace(' ')
    @test isspace('\t')
    @test isspace('\r')
    @test isspace('\u85')
...
@testset "Get full coverage of grapheme iterator functions" begin
